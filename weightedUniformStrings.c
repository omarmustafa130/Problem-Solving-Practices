...
char** weightedUniformStrings(char* s, int queries_count, int* queries, int* result_count) {
    int len = strlen(s);
    bool *weights = calloc(10000000, sizeof(bool));  // Allocate for 0 to 10^7
    int char_weight, uniform_weight;
    int currentLength = 1;
    for (int i = 0; i<len; i++)
    {
        char_weight = (s[i] - 'a')+1;
        uniform_weight = char_weight * currentLength;
        weights[uniform_weight] = true;
